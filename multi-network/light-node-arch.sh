...
#install pre-reqs
sudo pacman -S curl tar wget clang pkgconf openssl jq base-devel git make ncdu -y

#install go ver1.20.3
...
#go and variables in this session use .bash_profile to avoid dependancy clashes with .bashrc
source ~/.bash_profile
...
#reinstall keyrings for packages 
sudo pacman -Sy isl -y 
sudo pacman -Sy libmpc -y
sudo pacman -Sy gcc-libs glibc -y
sudo pacman -Sy linux-api-headers -y
