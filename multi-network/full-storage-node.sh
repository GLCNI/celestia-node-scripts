...
#install go ver1.19 sh
wget "https://raw.githubusercontent.com/GLCNI/celestia-node-scripts/main/multi-network/go-v19-install.sh"
chmod a+x go-v19-install.sh
./go-v19-install.sh
...
then
    ./cel-key list --node.type full --p2p.network $NETWORK --keyring-backend test
...
[Service]
User=$USER
ExecStart=$(which celestia) full start --keyring.accname $WALLET --p2p.network $NETWORK --core.ip $RPC_ENDPOINT --gateway --gateway.addr localhost 
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
...
#display logs 
echo "full storage node is now setup and running, allow some time to sync"
